                        ...
                        <tbody>
                             ...
                             <tr>
                                ...
                                {{-- <td class="text-center">
                                    <span><img src="{{ asset('storage/'.$service->image) }}" style="width: 100%;" class="profile-img" alt="avatar"></span>
                                </td> --}}
                                <td>{{ $service->title }}</td>
                                <td>{{ $service->title_en }}</td>
<td>{{$service->icon_class}}</td>
                                <td class="text-center">
                                    <a href=""><span class="shadow-none badge badge-{{ $service->status == 'Active' ? 'primary':'danger' }}">{{ $service->status }}</span></a>
                                </td>
                                <td class="text-center">
                                    <ul class="table-controls">
                                        ...
                                        {{-- <form action="{{ route('admin.service.destroy', $service->id) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger"onclick="confirmDelete({{ $service->id }})">Delete</button>
                                        </form> --}}
                                        <form id="deleteForm-{{ $service->id }}" action="{{ route('admin.service.destroy', $service->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
